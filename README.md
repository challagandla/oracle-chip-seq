...
10. Factor-specific `DiffBind` differential binding and narrow-factor motif enrichment with `monaLisa` and `JASPAR`
...
- Review `motif_enrichment.window_bp`, `background_multiplier`, and `seed`. The defaults use 200-bp narrow-factor windows and two deterministic, non-overlapping background windows per foreground interval.
...
Use the species-specific helper to download a genome FASTA, create its samtools `.fai`, build a Bowtie2 index, and generate chromosome sizes:
```bash
...
- `results/motifs/<factor>/` - full `monaLisa`/`JASPAR` enrichment table and top-motif CSV/PDF for narrow-peak factors
...
- Motif background windows preserve the foreground chromosome distribution and exclude blacklisted and foreground intervals, but they do not automatically match assay accessibility or mappability.
...
The pipeline code is MIT-licensed (see [LICENSE](LICENSE)). It bundles no third-party code or data; tools are installed from Conda and invoked by the workflow. See [THIRD_PARTY_LICENSES.md](THIRD_PARTY_LICENSES.md) for the dependency license and citation summary.
