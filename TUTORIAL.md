...
narrow consensus peaks          --> HOMER motif enrichment
...
│   └── motif_summary.R               motif table and figure
├── docs/
...
| HOMER | Tests sequence motifs enriched in narrow consensus peaks. |
...
1. a genome FASTA;
2. a chromosome-size table;
3. a complete Bowtie2 index prefix;
4. an assembly-matched blacklist BED;
5. a MACS3 genome size; and
6. a deepTools effective genome size written as a positive integer.

### Use the reference helper

After `bash setup.sh` has completed, the helper can download a primary-assembly
FASTA, build a Bowtie2 index, and create chromosome sizes:
...
checked human and mouse examples use the deepTools 150-bp values; the rat
...
[deepTools guidance](https://deeptools.readthedocs.io/en/3.5.6/content/feature/effectiveGenomeSize.html)
...
HOMER motif output is only available for factors configured as `narrow`.
...
├── motifs/<factor>/                 HOMER output and motif summary
...
For a narrow factor, the workflow runs HOMER on the factor consensus BED using
a 200 bp sequence window and tests motif lengths 8, 10, and 12.
...
The PDF shows up to 20 motifs ranked by `-log10(P-value)`. The full HOMER known
motif table is:
...
The default workflow does not run fixed-window motif analysis for broad
domains, where the biological interpretation is usually unclear.
...
- [ ] Genome, chromosome sizes, blacklist, and Bowtie2 index use one assembly.
...
- [deepTools 3.5.6 documentation](https://deeptools.readthedocs.io/en/3.5.6/)
