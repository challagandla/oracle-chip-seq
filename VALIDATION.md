...
This file records the repository checks run before the 2026-07-14 publication
update. It is not evidence that the workflow has been biologically validated on
a real ChIP-seq cohort. The GitHub Actions workflow reruns the portable static
and synthetic checks for each pushed commit and pull request.
...
Rscript -e 'parse(file="analysis/diffbind_analysis.R"); parse(file="analysis/motif_summary.R")'
python -m pytest -q
git diff --check
```
...
- Both R analysis scripts parsed with R 4.5.2.
- Pytest reported `28 passed` with Python 3.13.13 and Snakemake 9.22.0.
- The tests include Snakemake lint and a forced dry-run of a 136-job synthetic
...
All six solves completed successfully on 2026-07-14. A cross-platform solve
