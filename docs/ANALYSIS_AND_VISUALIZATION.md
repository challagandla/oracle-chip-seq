...
7. Narrow-factor consensus peaks drive open-source `monaLisa` enrichment against JASPAR vertebrate motifs. Motif enrichment is not a default broad-factor analysis.
...
### monaLisa/JASPAR motifs

Motif enrichment is available only for factors configured with `peak_mode: narrow`; the rule's wildcard is constrained to those factor slugs, so a broad-factor motif target is rejected. The workflow centers a 200 bp window on each interval midpoint in the narrow factor's consensus BED and tests JASPAR vertebrate motifs with `monaLisa`. The interval midpoint is not a retained MACS summit. The rule writes:
...
The factor consensus BED is the union of the two condition-specific, replicate-supported BEDs. The result therefore describes motifs associated with the combined factor region set; it is not a differential motif analysis and is not proof that a named transcription factor binds those sites.

The `motif_enrichment` configuration sets the exact window width, number of background windows per foreground window, and random seed. Background sampling preserves the foreground chromosome distribution, uses non-overlapping fixed-width windows, and excludes both blacklisted regions and the complete factor-consensus intervals. `monaLisa` then weights the sampled background to reduce GC and short k-mer composition differences. No precomputed background BED is required, but an assembly-matched FASTA, `.fai` index, and blacklist are required. This background does not automatically match assay accessibility or mappability; use a purpose-built background when those covariates matter. Infinite significance values are retained in the TSV/CSV and capped only for plotting. The workflow deliberately omits default fixed-window motif enrichment for broad domains, where a single local window is usually difficult to interpret.
...
The rule environment pins deepTools 3.5.5 and NumPy 1.26.x. NumPy stays below 2 because deepTools 3.5.x still uses the removed `numpy.NAN` alias. The upstream 3.5.5 documentation below was used for the command contracts and method guidance. The same environment pins MACS3 3.0.3, whose `callpeak` interface is documented by the linked upstream MACS3 reference.

- [deepTools 3.5.5 tool index](https://deeptools.readthedocs.io/en/3.5.5/content/list_of_tools.html), including direct documentation for [bamCoverage](https://deeptools.readthedocs.io/en/3.5.5/content/tools/bamCoverage.html), [bamCompare](https://deeptools.readthedocs.io/en/3.5.5/content/tools/bamCompare.html), [computeMatrix](https://deeptools.readthedocs.io/en/3.5.5/content/tools/computeMatrix.html), [plotHeatmap](https://deeptools.readthedocs.io/en/3.5.5/content/tools/plotHeatmap.html), and [plotProfile](https://deeptools.readthedocs.io/en/3.5.5/content/tools/plotProfile.html).
- [deepTools 3.5.6 effective-genome-size guidance](https://deeptools.readthedocs.io/en/3.5.6/content/feature/effectiveGenomeSize.html), including the newer GRCm39 read-length-specific values used by the example configuration.
- deepTools 3.5.5 documentation for [multiBigwigSummary](https://deeptools.readthedocs.io/en/3.5.5/content/tools/multiBigwigSummary.html), [plotCorrelation](https://deeptools.readthedocs.io/en/3.5.5/content/tools/plotCorrelation.html), [plotPCA](https://deeptools.readthedocs.io/en/3.5.5/content/tools/plotPCA.html), and [plotFingerprint QC metrics](https://deeptools.readthedocs.io/en/3.5.5/content/feature/plotFingerprint_QC_metrics.html).
...
- [monaLisa reference manual](https://bioconductor.org/packages/release/bioc/manuals/monaLisa/man/monaLisa.pdf), including binned enrichment and GC/short-k-mer background weighting.
- [JASPAR database](https://jaspar.elixir.no/), the open-access vertebrate motif collection used by the workflow.
