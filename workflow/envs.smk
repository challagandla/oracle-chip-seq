...
rule check_chipseq_env:
    ...
    shell:
        ...
        for tool in snakemake fastqc fastq_screen trim_galore bowtie2 samtools \
            ...
            multiqc wget gunzip gzip; do command -v "$tool" >/dev/null; done
        macs3 --version >/dev/null
        bamCoverage --version >/dev/null
        computeMatrix --version >/dev/null
        python -c 'import numpy, pandas, yaml; assert numpy.__version__.startswith("1.26.")'
        touch {output}
        """
...
rule check_r_analysis_env:
    ...
    shell:
        """
        mkdir -p .snakemake/setup-env-checks
        Rscript --vanilla -e 'suppressPackageStartupMessages({{library(DiffBind); library(monaLisa); library(JASPAR2020); library(TFBSTools); library(GenomicRanges); library(GenomeInfoDb); library(IRanges); library(rtracklayer); library(Rsamtools); library(Biostrings); library(SummarizedExperiment); library(BiocParallel); library(readr); library(dplyr); library(ggplot2)}})'
        touch {output}
        """
